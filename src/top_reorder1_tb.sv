...
module top_reorder1_tb;
    ...
    initial begin
        $dumpfile("top_reorder1.vcd"); $dumpvars(0, top_reorder1_tb);
        ...
        $display("\n== T3: reorder behind a busy burst (high-QoS overtakes) ==");
        // No fork (parallel drivers on one AR bus deadlock). Sequential:
        // issue a 4-beat read (occupies the engine), then queue id5 (low
        // QoS) and id6 (high QoS) behind it while RREADY is still low so
        // nothing drains. Then drain: the burst first, then id6 before id5.
        RREADY<=1'b0;
        ar_send(4'h4, SLAVE1_BASE+8'd8, 4'd3, 4'd8);   // 4-beat read, fills engine
        ar_send(4'h5, SLAVE1_BASE,       4'd0, 4'd1);  // low QoS, queued
        ar_send(4'h6, SLAVE1_BASE+8'd12, 4'd0, 4'd15); // high QoS, queued
        r_get(4, gid, gr); check(gid===4'h4, "T3 burst id4 drains first");
        r_get(1, gid, gr); check(gid===4'h6, "T3 next R is id6 (higher QoS overtakes id5)");
        r_get(1, gid, gr); check(gid===4'h5, "T3 last R is id5");
        $display("\n== T4: DECERR read (unmapped) ==");
        ar_send(4'h7, 8'h10, 4'd1); r_get(2, gid, gr);
        check(gr===RESP_DECERR && gid===4'h7, "T4 unmapped -> DECERR, all beats");


        $display("\n== T5: starvation promotion (aged low-QoS beats a fresh high-QoS) ==");
        // STARVE_LIMIT=8. To make the victim AGE, the engine must be genuinely
        // stalled - and RREADY=0 alone is not enough, because the master R skid
        // absorbs 2 beats before backpressuring. So the filler burst must be
        // LONGER than the skid depth (>2 beats): the engine streams 2 beats into
        // the skid, the skid fills, RREADY_DMUX drops, and the engine STALLS
        // mid-filler with RREADY low. While stalled, the queued victim ages.
        //   id7 : 4-beat filler (QoS 8) -> stalls the engine (skid full)
        //   id8 : victim, QoS 1  -> ages past STARVE_LIMIT -> promoted to 15
        //   id9 : fresh,  QoS 14 -> injected AFTER id8 aged; stays < limit
        // Without promotion id9(14) > id8(1) -> id9 wins. With promotion
        // id8(15) > id9(14) -> id8 wins. "id8 before id9" proves promotion.
        RREADY<=1'b0;
        ar_send(4'h7, SLAVE1_BASE,       4'd3, 4'd8);   // 4-beat filler, stalls engine
        ar_send(4'h8, SLAVE1_BASE+8'd4,  4'd0, 4'd1);   // victim, begins aging
        repeat (STARVE_LIMIT + 4) @(posedge ACLK);      // victim ages past 8 while engine stalled
        ar_send(4'h9, SLAVE1_BASE+8'd8,  4'd0, 4'd14);  // fresh newcomer (age 0)
        r_get(4, gid, gr); check(gid===4'h7, "T5 filler id7 drains first (4 beats)");
        r_get(1, gid, gr); check(gid===4'h8, "T5 aged id8 promoted -> beats fresh id9");
        r_get(1, gid, gr); check(gid===4'h9, "T5 id9 last");
